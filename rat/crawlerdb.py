...
from rat.backlinkprocessor import BacklinkData
...
class WebsiteCrawlerDatabase:
    ...
    def get_discovered_subdomains(self) -> List[str]:
        """Get list of discovered subdomains from crawled pages."""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()

        cursor.execute('''
            SELECT DISTINCT url FROM crawled_pages
            WHERE url LIKE '%.%' AND instr(url, '.') > 0
    ...
    def get_domain_authority_scores(self) -> Dict[str, float]:
        ...
        cursor.execute('SELECT domain, authority_score FROM domain_scores ORDER BY authority_score DESC')
    ...
    def get_pagerank_scores(self) -> Dict[str, float]:
        ...
        cursor.execute('SELECT url, pagerank_score FROM pagerank_scores ORDER BY pagerank_score DESC')
        results = dict(cursor.fetchall())
    ...
    def cleanup_old_data(self, days_old: int = 30) -> int:
        """Clean up old crawl data older than specified days. Returns number of records deleted."""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        # Delete old crawled pages (keep recent ones)
        cursor.execute('''
        ...
        # Delete old backlinks (keep recent ones)
        cursor.execute('''
        ...
        deleted_backlinks = cursor.rowcount

        # Delete old crawl errors
        cursor.execute('''
        ...
        total_deleted = deleted_pages + deleted_backlinks + deleted_errors
        return total_deleted

    def get_recent_backlinks(self, hours: int = 24) -> List[Dict]:
        """Get backlinks from the last specified hours."""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
